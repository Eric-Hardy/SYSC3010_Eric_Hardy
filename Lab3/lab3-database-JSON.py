...
#Extend to make a new table in the database
dbconnect = sqlite3.connect("database.db")
dbconnect.row_factory = sqlite3.Row
cursor = dbconnect.cursor()
cursor.execute('''CREATE TABLE wind (city TEXT, windSpeed NUMERIC)''')
dbconnect.commit()
cursor.execute('''INSERT INTO wind values("%s", %d)''' %(city, current["speed"]))
dbconnect.commit()
